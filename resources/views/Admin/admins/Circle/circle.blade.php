    	...
    	<div class="row">
        	<div class="col-lg-12">
                <h1 class="page-header">Admins
                    <a href="{{ url('admin/addCircleAdmin') }}" title="Add new admin">
                ...
                <div class="panel panel-default">
                    ...
                    <div class="panel-body">
                        <table width="100%" class="table table-striped table-bordered table-hover" id="dataTables-example">
                            ...
                            <tbody>
                            @if(count($circleAdminList) > 0)
                            	@foreach($circleAdminList as $index => $admin)
                                    <tr class="odd gradeX">
                                        <td>{{$index + 1}}</td>
                                        <td>{{$admin['name']}}</td>
                                        <td>{{$admin['circleName']}}</td>
                                        <td>{{$admin['email']}}</td>
                                        <td>{{$admin['phone_no']}}</td>
                                        <td>{{$admin['designation']}}</td>
                                        <td>{{$admin['status']}}</td>
                                        <td>
                                        	<a href="{{ url('admin/editCircleAdmin?id=') }}{{$admin['id']}}" title="Edit"><i class="fa fa-edit fa-fw"></i></a>
...
<script>
    $(".circleDelete").on("click", function(){
        ...
        if (decision) {
            ...
            window.location.href = "{{URL::to('admin/deleteCircleAdmin?id=')}}" + adminId
        }
    });
</script>
