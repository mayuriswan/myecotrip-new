            ...
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">Landscape
                        <a href="{{url('/cms/ecotrails/landscape/create')}}" title="Add new admin">
                            ...
                            <tbody class="tblFont">
                                @foreach($data as $key => $row)
                                    <tr>
                                        ...
                                        <td>{{$row['seo_url']}}</td>
                                        <td>
                                            <img class="navbarLogo" src="{{asset($row['logo']) }}" alt="I{{$row['name']}}" title="Image Title" />
                                        </td>
                                        <td>{{$row['display_order_no']}}</td>
                                        <td>{{$row['status']}}</td>
                                        
                                        <td>
                                        	<a href="{{ url('cms/ecotrails/landscape')}}/{{$row['id']}}/edit" title="Edit"><i class="fa fa-edit fa-fw"></i> Edit</a>
                                        </td>
                                        <td>
...
<script>
    ...
    $(".deleteRow").on("click", function(){
        ...
        if (decision) {
          ...
          type: "DELETE",
          url: "{{URL::to('cms/ecotrails/landscape')}}/" + id,
