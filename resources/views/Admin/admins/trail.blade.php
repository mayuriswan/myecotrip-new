    	...
    	<div class="row">
        	<div class="col-lg-12">
                <h1 class="page-header">Admins
                    <a href="{{ url('admin/addTrailAdmin') }}" title="Add new admin">
                ...
                <div class="panel panel-default">
                    ...
                    <div class="panel-body">
                        <table width="100%" class="table table-striped table-bordered table-hover" id="dataTables-example">
                            ...
                            <tbody>
                            @if(count($trailAdminList) > 0)
                            	@foreach($trailAdminList as $index => $admin)
                                    <tr class="odd gradeX">
                                        <td>{{$index + 1}}</td>
                                        <td>{{$admin['name']}}</td>
                                        <td>{{$admin['trailName']}}</td>
                                        <td>{{$admin['email']}}</td>
                                        <td>{{$admin['phone_no']}}</td>
                                        <td>{{$admin['designation']}}</td>
                                        <td>{{$admin['status']}}</td>
                                        <td>
                                        	<a href="{{ url('admin/editTrailAdmin?id=') }}{{$admin['id']}}" title="Edit"><i class="fa fa-edit fa-fw"></i></a>
                                        	
                                            <a value="{{$admin['id']}}" class="trailDelete" title="Delete"><i class="fa fa-trash-o" aria-hidden="true"></i></a>
                                            
                                            <!-- <a href="{{ url('admin/viewTrail?id=') }}{{$admin['id']}}" title="View"><i class="fa fa-eye" aria-hidden="true"></i></a> -->
                                        </td>
                                    </tr>
							    @endforeach				             	                             
							@else
...
<script>
    $(".trailDelete").on("click", function(){
        ...
        if (decision) {
            ...
            window.location.href = "{{URL::to('admin/deleteTrailAdmin?id=')}}" + trailId
        }
    });
</script>
