...
<script src="{{ URL::asset('assets/js/multiselect.js')}}"></script>
    ...
    <div id="page-wrapper">
        ...
        @endif
        <form action="{{url('/')}}/admin/downloadBirdSanctoryReport" method="POST">
        	<div class="row">
        	...
        	<div class="container">
			   	<div class="row">
                    <div class="col-sm-3">
                        ...
                        <select id="dates-field2" name="birdSanctuaryIds[]" class="multiselect-ui form-control" multiple="multiple" required>
                            @foreach($sanctoryList as $id => $sanctory)
                                <option value="{{$id}}">{{$sanctory}}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-sm-2">
