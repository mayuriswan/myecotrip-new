            ...
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">Upcoming trails
                        <a href="{{url('cms/ecotrails/coming-soon/create')}}" title="Add new">
                            ...
                            <tbody class="tblFont">
                                @foreach($data as $key => $row)
                                    <tr>
                                        ...
                                        <td>{{$row['name']}}</td>
                                        <td>{{$row['shortDesc']}}</td>
                                        <td>{{$row['googleSearchText']}}</td>
                                        <td>{{$row['status']}}</td>
                                        
                                        <td>
                                        	<a href="{{ url('cms/ecotrails/coming-soon')}}/{{$row['id']}}/edit" title="Edit"><i class="fa fa-edit fa-fw"></i> Edit</a>
                                        </td>
                                        <td>
...
<script>
    ...
    $(".deleteRow").on("click", function(){
        ...
        if (decision) {
          ...
          type: "DELETE",
          url: "{{URL::to('cms/ecotrails/coming-soon')}}/" + id,
