...
<div id="page-wrapper">
    <div class="container-fluid">
      <div class="row">
         <div class="col-lg-12">
                <h1 class="page-header">Upcoming Trails
                    <a href="{{ url('admin/addTrailUpcoming') }}" title="Add new">
                    ...
                    <div class="panel-body">
                        <table width="100%" class="table table-striped table-bordered table-hover" id="dataTables-example">
                            ...
                            @if(count($upcomingList) > 0)
                              @foreach($upcomingList as $index => $trail)
                                    <tr class="odd gradeX">
                                        <td>{{$index + 1}}</td>
                                        <td>{{$trail['name']}}</td>
                                        <td>{{$trail['shortDesc']}}</td>
                                        <td>{{$trail['status']}}</td>
                                        <td>
                                          <a href="{{ url('admin/editTrailUpcoming') }}/{{$trail['id']}}" title="Edit"><i class="fa fa-edit fa-fw"></i></a>
                                          
                                            <a value="{{$trail['id']}}" class="trailDelete" title="Delete"><i class="fa fa-trash-o" aria-hidden="true"></i></a>
                                        </td>
                                    </tr>
                         @endforeach                                                        
                     @else
...
<script>
    $(".trailDelete").on("click", function(){
        ...
        if (decision) {
            ...
            window.location.href = "{{URL::to('admin/deleteTrailUpcoming')}}" + '/' +trailId
        }
    });
</script>
