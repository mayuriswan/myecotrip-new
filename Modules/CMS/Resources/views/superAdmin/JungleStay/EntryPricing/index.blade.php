            ...
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">Entry Pricing
                        <a href="{{url('/cms/jungle-stay/entry/create')}}" title="Add new">
                            ...
                            <tbody class="tblFont">
                                @foreach($data as $key => $row)
                                    <tr>
                                        <td>{{++$key}}</td>
                                        <td>{{$row['name']}}</td>
                                        <td>{{$row['master']}} {{$row['shortDesc']}}</td>
                                        <td>{{$row['price']}}</td>
                                        <td>
        ...
        if (decision) {
        	$.ajax({
    		 	headers: {
			  	...
			  	type: "DELETE",
			  	url: "{{URL::to('cms/jungle-stay/entry')}}/" + id,
                success: function(data, textStatus, jqXHR){
	                console.log(jqXHR.status);
	                window.location.href= "{{url('/')}}" + '/cms/jungle-stay/entry';
	            }
			});
        }
    });
