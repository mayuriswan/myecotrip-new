    	...
    	<div class="row">
        	<div class="col-lg-12">
                <h1 class="page-header">Admins
                    <a href="{{ url('admin/addParkAdmin') }}" title="Add new admin">
                ...
                <div class="panel panel-default">
                    ...
                    <div class="panel-body">
                        <table width="100%" class="table table-striped table-bordered table-hover" id="dataTables-example">
                            ...
                            <tbody>
                            @if(count($parkAdminList) > 0)
                            	@foreach($parkAdminList as $index => $admin)
                                    <tr class="odd gradeX">
                                        <td>{{$index + 1}}</td>
                                        <td>{{$admin['name']}}</td>
                                        <td>{{$admin['parkName']}}</td>
                                        <td>{{$admin['email']}}</td>
                                        <td>{{$admin['phone_no']}}</td>
                                        <td>{{$admin['designation']}}</td>
                                        <td>{{$admin['status']}}</td>
                                        <td>
                                        	<a href="{{ url('admin/editParkAdmin?id=') }}{{$admin['id']}}" title="Edit"><i class="fa fa-edit fa-fw"></i></a>
                                        	
                                            <a value="{{$admin['id']}}" class="parkDelete" title="Delete"><i class="fa fa-trash-o" aria-hidden="true"></i></a>
                                        </td>
                                    </tr>
							    @endforeach				             	                             
							@else
...
<script>
    $(".parkDelete").on("click", function(){
        ...
        if (decision) {
            ...
            window.location.href = "{{URL::to('admin/deletePrailAdmin?id=')}}" + adminId
        }
    });
</script>
