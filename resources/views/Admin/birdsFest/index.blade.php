...
@section('content')
    ...
    <div id="page-wrapper">
        <div class="container-fluid">
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">Birds Fest
                        <a href="{{ url('admin/addBirdsFest') }}" title="Add Birds Fest">
                        ...
                        <div class="panel-body">
                            <table width="100%" class="table table-striped table-bordered table-hover" id="dataTables-example">
                                ...
                                @if(count($birdsFestList) > 0)
                                    @foreach($birdsFestList as $index => $birdsFest)
                                        <tr class="odd gradeX">
                                            <td>{{$index + 1}}</td>
                                            <td>{{$birdsFest['eventName']}}</td>
                                            <td>{{$birdsFest['name']}}</td>
                                            <td>
                                                <a href="{{ url('admin/editBirdsFest?id=') }}{{$birdsFest['id']}}"  title="Edit"><i class="fa fa-edit fa-fw"></i></a>
                                                &nbsp;
                                                <a value="{{$birdsFest['id']}}" class="birdsFestDelete" title="Delete"><i class="fa fa-trash-o" aria-hidden="true"></i></a>
                                            </td>
                                        </tr>
                                    @endforeach
                                @else
    ...
    <script>
        $(".birdsFestDelete").on("click", function(){
            var decision = confirm("Do you want to delete this item?");

            if (decision) {
                var birdsFestId = $(this).attr('value');
                window.location.href = "{{URL::to('admin/deleteBirdsFest?id=')}}" + birdsFestId
            }
        });
    </script>
