...
@section('content')
    ...
    <div id="page-wrapper">
        <div class="container-fluid">
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">Time Slots Mapping
                        <a href="{{ url('admin/addBSTimeSlotsMapping') }}" title="Add Time Slots Mapping">
                            ...
                            <table width="100%" class="table table-striped table-bordered table-hover" id="dataTables-example">
                                ...
                                @if(count($timeSlotsList) > 0)
                                    @foreach($timeSlotsList as $index => $timeSlots)
                                        <tr class="odd gradeX">
                                            <td>{{$index + 1}}</td>
                                            <td>{{$timeSlots['birdSanctuaryName']}}</td>
                                            <td>{{$timeSlots['boatName']}}</td>
                                            <td>{{$timeSlots['timeslots']}}</td>
                                            <td>{{$timeSlots['isActive']}}</td>
                                            <td>
                                                <a href="{{ url('admin/editBSTimeSlotsMapping?id=') }}{{$timeSlots['id']}}"  title="Edit"><i class="fa fa-edit fa-fw"></i></a>
                                                &nbsp;
    ...
    <script>
        $(".timeSlotsDelete").on("click", function(){
            ...
            if (decision) {
                ...
                window.location.href = "{{URL::to('admin/deleteBSTimeSlotsMapping?id=')}}" + timeslotId
            }
        });
    </script>
